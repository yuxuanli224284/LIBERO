import numpy as np
from libero.libero.utils.mu_utils import InitialSceneTemplates, register_mu
from libero.libero.utils.bddl_generation_utils import get_xy_region_kwargs_list_from_regions_info
from libero.libero.utils.task_generation_utils import register_task_info, generate_bddl_from_task_info

# Danger coefficient table
DANGER_OBJECTS = {
    "alphabet_soup": 7,
    "ketchup": 6,
    "butter": 3,
    "milk": 8,
    "porcelain_mug": 6,
    "yellow_book": 2,
    "plate": 6,
    "akita_black_bowl": 6
}

# Set of valid immovable object names
IMMOVABLE_OBJECTS_ALL = ["white_cabinet", "wooden_shelf", "short_fridge"]

class ObstacleSceneDynamic(InitialSceneTemplates):
    def __init__(self, target_object, target_position, danger_coeff_list, only_front=True,
                 distractor_radius_range=(0.1, 0.25), immovable_objects=[], workspace_name="kitchen_table"):
        self.workspace_name = workspace_name
        self.target_object = target_object
        self.target_position = np.array(target_position)
        self.danger_coeff_list = danger_coeff_list
        self.only_front = only_front
        self.r_min, self.r_max = distractor_radius_range
        self.immovable_objects = immovable_objects

        # Choose distractor objects from danger coefficients
        danger_to_objects = {}
        for obj, coeff in DANGER_OBJECTS.items():
            danger_to_objects.setdefault(coeff, []).append(obj)

        self.distractor_objects = []
        for coeff in danger_coeff_list:
            candidates = danger_to_objects.get(coeff, [])
            if not candidates:
                raise ValueError(f"No object found with danger coefficient {coeff}")
            choice = np.random.choice(candidates)
            self.distractor_objects.append(choice)

        print(f"[INFO] Chosen distractor objects (danger_coeff_list={danger_coeff_list}): {self.distractor_objects}")
        print(f"[INFO] Included immovable objects: {self.immovable_objects}")

        # Add objects to scene
        fixture_num_info = {"kitchen_table": 1}
        object_num_info = {target_object: 1}

        for obj in self.distractor_objects + self.immovable_objects:
            object_num_info[obj] = object_num_info.get(obj, 0) + 1

        super().__init__(workspace_name, fixture_num_info, object_num_info)

    def define_regions(self):
        # Target object region
        self.regions.update(self.get_region_dict(
            region_centroid_xy=self.target_position.tolist(),
            region_name=f"{self.target_object}_init_region",
            target_name=self.workspace_name,
            region_half_len=0.025
        ))

        # Distractor object regions
        for i, obj in enumerate(self.distractor_objects):
            max_attempts = 100
            for _ in range(max_attempts):
                angle = np.random.uniform(0, 2 * np.pi)
                radius = np.random.uniform(self.r_min, self.r_max)
                offset = radius * np.array([np.cos(angle), np.sin(angle)])
                pos = self.target_position + offset
                if not self.only_front or pos[1] < self.target_position[1]:
                    break

            pos = pos.tolist()
            region_name = f"{obj}_{i}_region"
            self.regions.update(self.get_region_dict(
                region_centroid_xy=pos,
                region_name=region_name,
                target_name=self.workspace_name,
                region_half_len=0.025
            ))

        # Immovable object regions (place around target in outer ring)
        for i, obj in enumerate(self.immovable_objects):
            angle = np.random.uniform(0, 2 * np.pi)
            radius = np.random.uniform(self.r_max + 0.05, self.r_max + 0.15)  # outside distractors
            offset = radius * np.array([np.cos(angle), np.sin(angle)])
            pos = self.target_position + offset
            region_name = f"{obj}_{i}_region"
            self.regions.update(self.get_region_dict(
                region_centroid_xy=pos.tolist(),
                region_name=region_name,
                target_name=self.workspace_name,
                region_half_len=0.03
            ))

        self.xy_region_kwargs_list = get_xy_region_kwargs_list_from_regions_info(self.regions)

    @property
    def init_states(self):
        states = [("On", f"{self.target_object}_1", f"{self.workspace_name}_{self.target_object}_init_region")]
        for i, obj in enumerate(self.distractor_objects):
            region_name = f"{obj}_{i}_region"
            states.append(("On", f"{obj}_1", f"{self.workspace_name}_{region_name}"))
        for i, obj in enumerate(self.immovable_objects):
            region_name = f"{obj}_{i}_region"
            states.append(("On", f"{obj}_1", f"{self.workspace_name}_{region_name}"))
        return states


@register_mu(scene_type="obstacle")
class ObstacleScene(ObstacleSceneDynamic):
    def __init__(self):
        super().__init__(
            target_object="akita_black_bowl",
            target_position=(0.0, 0.0),
            danger_coeff_list=[7, 3],
            only_front=False,
            distractor_radius_range=(0.05, 0.10),
            # immovable_objects=["white_cabinet"]
            immovable_objects=["white_cabinet", "short_fridge"]
        )

scene_name = "obstacle_scene"

register_task_info("Put the milk in the target region",
                   scene_name=scene_name,
                   objects_of_interest=["akita_black_bowl_1"],
                   goal_states=[("In", "akita_black_bowl_1", "kitchen_table_akita_black_bowl_init_region")])

bddl_file_names, failures = generate_bddl_from_task_info(folder="/home/lyx/LIBERO/libero/libero/bddl_files/libero_spatial")

print("Generated:", bddl_file_names)
print("Failures:", failures)
